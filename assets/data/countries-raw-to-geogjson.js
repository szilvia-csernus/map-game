...
const geojson = () => {
	...
	const features = countries.map((country) => {
		if (
			...
			const { CapitalLongitude, CapitalLatitude, ...props } = country;
			return {
				type: 'Feature',
				geometry: {
					type: 'Point',
					coordinates: [CapitalLongitude, CapitalLatitude],
				},
				properties: { ...props },
			};
		}
	...
	const filtered = features.filter((obj) => obj);
	geo.features = filtered;
	return geo
};

// Transforming data to geojson format

const data = JSON.stringify(geojson());
const fs = require('fs');
fs.writeFile('countries.geojson', data, (err) => {
	if (err) {
		console.err(err);
	} else {
		data;
